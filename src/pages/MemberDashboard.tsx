...
const MemberDashboard = () => {
  ...
  const { data: myDepartments } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("department_members").select("department_id, departments(name)").eq("profile_id", profile!.id);
      if (error) throw error;
      return data;
    },
    enabled: !!profile,
  ...
  const { data: attendance } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("attendance_logs").select("*, programs(name)").eq("profile_id", profile!.id).order("scan_time", { ascending: false }).limit(10);
      if (error) throw error;
      return data;
    },
    enabled: !!profile,
  ...
  const { data: givingRecords } = useQuery({
    queryKey: ["my-giving", profile?.id],
    queryFn: async () => {
      const { data, error } = await supabase.from("giving_records").select("*").eq("profile_id", profile!.id).order("date", { ascending: false }).limit(20);
      if (error) throw error;
      return data;
    },
    enabled: !!profile,
  ...
  const { data: familyData } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("family_members").select("*, families(family_name, household_address)").eq("profile_id", profile!.id);
      if (error) throw error;
      return data;
    },
    enabled: !!profile,
  ...
  const totalGiving = givingRecords?.reduce((sum, r: any) => sum + Number(r.amount), 0) || 0;
    ...
    <div className="min-h-screen bg-background flex">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 gradient-navy text-primary-foreground transform transition-transform duration-200 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="p-3 space-y-1">
          {navItems.map((item) => (
            <a key={item.id} href={`#${item.id}`} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm hover:bg-primary-foreground/10 text-primary-foreground/80 transition-colors">
              <item.icon className="h-4 w-4" />{item.label}
            </a>
          ))}
        </nav>
        <div className="absolute bottom-4 left-3 right-3">
          <button onClick={async () => { await signOut(); navigate("/"); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-primary-foreground/60 hover:bg-primary-foreground/10 transition-colors">
      ...
      <main className="flex-1 lg:ml-64">
        ...
        <div className="p-4 lg:p-8 space-y-6">
                ...
                {profile ? (
                  <div className="flex flex-col sm:flex-row gap-6">
                    <Avatar className="h-20 w-20 border-2 border-muted">
                      ...
                      <AvatarFallback className="bg-primary/10 text-primary font-bold text-xl">{getInitials(profile.full_name)}</AvatarFallback>
                    </Avatar>
                    <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm flex-1">
                      ...
                      <ProfileField label="Birthday" value={profile.birth_month && profile.birth_day ? `${["","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][profile.birth_month]} ${profile.birth_day}` : profile.date_of_birth} />
                      <ProfileField label="Marital Status" value={profile.marital_status} />
                      {(profile as any).spouse_name && <ProfileField label="Spouse" value={(profile as any).spouse_name} />}
                      {(profile as any).member_code && <ProfileField label="Member ID" value={(profile as any).member_code} />}
          ...
          <section id="attendance">
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-3"><CardTitle style={{ fontFamily: 'var(--font-display)' }}>My Attendance</CardTitle></CardHeader>
              <CardContent>
                      ...
                      <div key={log.id} className="flex items-center justify-between py-2 border-b border-muted last:border-0">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" /> {new Date(log.scan_time).toLocaleDateString()}</p>
                          </div>
                        </div>
                        <Badge variant="outline" className="bg-primary/10 text-primary text-xs">{log.status}</Badge>
                      </div>
                    ))}
                  </div>
                ) : (
          ...
          <section id="giving">
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-3 flex flex-row items-center justify-between">
                <CardTitle style={{ fontFamily: 'var(--font-display)' }}>My Giving</CardTitle>
                <Badge variant="secondary" className="text-sm">Total: ₦{totalGiving.toLocaleString()}</Badge>
              </CardHeader>
              <CardContent>
                {givingRecords && givingRecords.length > 0 ? (
                  <div className="space-y-3">
                    {givingRecords.map((record: any) => (
                      <div key={record.id} className="flex items-center justify-between py-2 border-b border-muted last:border-0">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-full bg-secondary/10 flex items-center justify-center"><DollarSign className="h-4 w-4 text-secondary" /></div>
                          <div>
                            <p className="text-sm font-medium capitalize">{record.giving_type}</p>
                            <p className="text-xs text-muted-foreground">{new Date(record.date).toLocaleDateString()} • {record.payment_method}</p>
                          </div>
                        </div>
                        <span className="font-semibold text-sm">₦{Number(record.amount).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-muted-foreground text-sm">No giving records yet.</p>
                )}
              </CardContent>
            </Card>
          </section>
          ...
          <section id="departments">
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-3"><CardTitle style={{ fontFamily: 'var(--font-display)' }}>My Departments</CardTitle></CardHeader>
              <CardContent>
                {myDepartments && myDepartments.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {myDepartments.map((dm: any) => (
                      <Badge key={dm.department_id} variant="secondary" className="px-3 py-1.5"><Building2 className="h-3 w-3 mr-1.5" />{dm.departments?.name || "Department"}</Badge>
                    ))}
                  </div>
                ) : (
          ...
          <section id="family">
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-3"><CardTitle style={{ fontFamily: 'var(--font-display)' }}>My Family</CardTitle></CardHeader>
              <CardContent>
          ...
          {profile?.qr_code && (
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-3"><CardTitle style={{ fontFamily: 'var(--font-display)' }}>My QR Code</CardTitle></CardHeader>
